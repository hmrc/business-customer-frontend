...
import controllers.nonUKReg.{AgentRegisterNonUKClientController, BusinessRegController, NRLQuestionController, OverseasCompanyRegController, PaySAQuestionController, UpdateNonUKBusinessRegistrationController}
import controllers.{ApplicationController, BackLinkController, BusinessCustomerController, BusinessVerificationController, BusinessVerificationControllerImpl, ExternalLinkController, HomeController, ReviewDetailsController}
...
class Wiring extends Module {
  ...
  private def bindControllers: Seq[Binding[_]] = Seq(
    ...
    bind(classOf[BusinessRegController]).toSelf,
    bind(classOf[BusinessVerificationController]).to(classOf[BusinessVerificationControllerImpl]),
