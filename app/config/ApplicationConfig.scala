...
class ApplicationConfig @Inject()(val conf: ServicesConfig,
  ...
  lazy val analyticsHost: String = Try(conf.getString("google-analytics.host")).getOrElse("auto")
  ...
  def serviceSignOutUrl(service: String): String = {
    Try {
      conf.getString(s"delegated-service.${service.toLowerCase}.sign-out-url")
    } match {
      case Success(s) => s
      case _ => logoutUrl
    }
  ...
  def agentConfirmationPath(service:String): String = {
    Try {
      conf.getString(s"microservice.services.${service.toLowerCase}.agentConfirmationUrl")
    } match {
      case Success(s) => s
      case _ => "/ated-subscription/agent-confirmation"
    }
  }
