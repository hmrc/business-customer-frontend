...
class OverseasCompanyRegDetailsController @Inject()(val authConnector: AuthConnector,
  ...
  extends FrontendController(mcc) with AuthActions with BackLinkController with OverseasCompanyRegUtils {

  implicit val appConfig: ApplicationConfig = config
  implicit val executionContext: ExecutionContext = mcc.executionContext
  val controllerId: String = "OverseasCompanyRegDetailsController"
    ...
    authorisedFor(service) { implicit authContext =>
      RedirectUtils.getRelativeOrBadRequestOpt(redirectUrl) { newUrl =>
        for {
          ...
          overseasId <- businessRegistrationCache.fetchAndGetCachedDetails[OverseasCompany](OverseasRegDetailsId)
        } yield {
          overseasId match {
            case Some(oversea) => {
  ...
  def register(service: String, addClient: Boolean, redirectUrl: Option[RedirectUrl] = None): Action[AnyContent] = Action.async { implicit request =>
    authorisedFor(service){ implicit authContext =>
      BusinessRegistrationForms.validateNonUkIdentifiers(overseasCompanyRegForm.bindFromRequest()).fold(
        formWithErrors => {
        ...
        overseasCompany => for {
          ...
          _ <- cachedBusinessReg match {
            ...
            case None => throw new RuntimeException(s"[OverseasCompanyRegDetailsController][send] - service :$service. Error : No Cached BusinessRegistration")
          }
          redirectPage <- redirectUrl match {
            case Some(x) => RedirectUtils.getRelativeOrBadRequest(x) {newUrl => redirectToExternal(newUrl, Some(
              controllers.nonUKReg.routes.OverseasCompanyRegDetailsController.view(service, addClient, Some(x)).url
            ))}
            case None => redirectWithBackLink(
              ...
              Some(controllers.nonUKReg.routes.OverseasCompanyRegDetailsController.view(
