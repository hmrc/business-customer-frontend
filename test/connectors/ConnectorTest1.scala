...
trait ConnectorTest1 extends FutureAwaits with DefaultAwaitTimeout with MockitoSugar {
  ...
  def executePut[A](body: JsValue) = {
    val mockPutRequestBuilder: RequestBuilder = mock[RequestBuilder]
    when(mockPutRequestBuilder.setHeader(any[(String, String)])).thenReturn(mockPutRequestBuilder)
    when(mockHttpClient.put(any[URL])(any[HeaderCarrier])).thenReturn(mockPutRequestBuilder)
    when(mockPutRequestBuilder.withBody(ArgumentMatchers.eq(body))(any(), any(), any())).thenReturn(mockPutRequestBuilder)
    mockPutRequestBuilder.execute[A](any[HttpReads[A]], any[ExecutionContext])
  }
