...
class BusinessVerificationControllerSpec extends PlaySpec with GuiceOneServerPerSuite with MockitoSugar with Injecting {
    ...
    "when selecting Sole Trader option" must {
      ...
      "add additional form fields to the screen for entry" in new Setup {
        businessLookupWithAuthorisedUser(controller, "SOP", "AWRS") { result =>
          ...
          document.getElementsByAttributeValue("for", "saUTR").text() must include("What is your Self Assessment Unique Taxpayer Reference (UTR)?")
          document.select("#saUTR-hint").text() must include("This is 10 numbers, for example 1234567890. It will be on tax returns and other letters about Corporation Tax. It may be called ‘reference’, ‘UTR’ or ‘official use’.")
      ...
      "display correct heading for AGENT selecting Sole Trader" in new Setup {
        businessLookupWithAuthorisedAgent(controller, "SOP") { result =>
          ...
          document.getElementsByAttributeValue("for", "lastName").text() must be("Last name")
          document.getElementsByAttributeValue("for", "saUTR").text() must include("What is your Self Assessment Unique Taxpayer Reference (UTR)?")
          document.select("#saUTR-hint").text() must be("This is 10 numbers, for example 1234567890. It will be on tax returns and other letters about Corporation Tax. It may be called ‘reference’, ‘UTR’ or ‘official use’.")
          document.getElementById("saUTR").attr("type") must be("text")
          document.getElementById("submit").text() must include("Continue")
    ...
    "when selecting Limited Company option" must {
      ...
      "add additional form fields to the screen for entry" in new Setup {
        businessLookupWithAuthorisedUser(controller, "LTD") { result =>
          ...
          document.select("#cotaxUTR-hint").text() must be("This is 10 numbers, for example 1234567890. It will be on tax returns and other letters about Corporation Tax. It may be called ‘reference’, ‘UTR’ or ‘official use’.")
    ...
    "when selecting Ordinary business partnership" must {
      ...
      "add additional form fields to the screen for entry" in new Setup {
        businessLookupWithAuthorisedUser(controller, "OBP") { result =>
          ...
          document.getElementsByAttributeValue("for", "psaUTR").text() must include("What is your Partnership Self Assessment Unique Taxpayer Reference (UTR)")
          document.select("#psaUTR-hint").text() must be("This is 10 numbers, for example 1234567890. It will be on tax returns and other letters about Corporation Tax. It may be called ‘reference’, ‘UTR’ or ‘official use’.")
  ...
  private def willSaveBackLink(backLink : String) =
    when(mockBackLinkCache.saveBackLink(
      ...
      ArgumentMatchers.eq(Some(backLink)))(ArgumentMatchers.any(), ArgumentMatchers.any()))
  ...
  def businessVerificationWithAuthorisedUser(controller: BusinessVerificationController)
    ...
    when(mockBackLinkCache.fetchAndGetBackLink(ArgumentMatchers.any())(ArgumentMatchers.any(), ArgumentMatchers.any())).thenReturn(Future.successful(Some("/ated-subscription/appoint-agent")))
