...
class OverseasCompanyRegControllerSpec extends GuiceTestApp with MockitoSugar {
  ...
  object TestController
      extends OverseasCompanyRegController(
        mockAuthConnector,
        mockBackLinkCache,
        appConfig,
  ...
  "OverseasCompanyRegController" must {
    ...
    "send" must {
      ...
      val reviewDetails =
        ReviewDetails("ACME", Some("Unincorporated body"), regAddress, "sap123", "safe123", isAGroup = false, directMatch = false, Some("agent123"))
      ...
      "validate form" must {
        ...
        "select a radio button" in {
          registerWithAuthorisedUserSuccess(
            FakeRequest("POST", "/").withFormUrlEncodedBody(Map("hasBusinessUniqueId" -> "").toSeq: _*),
            "ATED",
            Some(businessReg),
            overseasDetails,
        ...
        "not be empty" in {
          registerWithAuthorisedUserSuccess(
            FakeRequest("POST", "/").withFormUrlEncodedBody(
              Map("hasBusinessUniqueId" -> "true", "businessUniqueId" -> "", "issuingInstitution" -> "", "issuingCountry" -> "").toSeq: _*),
            "ATED",
            Some(businessReg),
            overseasDetails,
            reviewDetails
            ...
            document
              ...
              .text() mustBe "Enter an overseas company registration number Enter the country that issued the overseas company registration number Enter an institution that issued the overseas company registration number"
            document.getElementById("businessUniqueId-error").text() mustBe "Error: Enter an overseas company registration number"
            document
              .getElementById("issuingCountry-error")
              .text() mustBe "Error: Enter the country that issued the overseas company registration number"
            document
              .getElementById("issuingInstitution-error")
              .text() mustBe "Error: Enter an institution that issued the overseas company registration number"
          }
        }
        ...
        val formValidationInputDataSet: Seq[(Map[String, String], TestMessage, ErrorMessage)] = Seq(
          (
            Map(
              ...
              "businessUniqueId"    -> s"${"a" * 61}",
              "issuingInstitution"  -> "some-institution",
              "issuingCountry"      -> "FR"),
            "businessUniqueId must be maximum of 60 characters",
            "The overseas company registration number cannot be more than 60 characters"),
          (
            Map("hasBusinessUniqueId" -> "true", "businessUniqueId" -> "some-id", "issuingInstitution" -> s"${"a" * 41}", "issuingCountry" -> "FR"),
            "issuingInstitution must be maximum of 40 characters",
            "The institution that issued the overseas company registration number cannot be more than 40 characters"),
          (
            Map(
              "hasBusinessUniqueId" -> "true",
              "businessUniqueId"    -> "some-id",
              "issuingInstitution"  -> "some-institution",
              "issuingCountry"      -> "GB"),
            "show an error if issuing country is selected as GB",
        ...
        formValidationInputDataSet.foreach { data =>
          s"${data._2}" in {
            registerWithAuthorisedUserSuccess(
              FakeRequest("POST", "/").withFormUrlEncodedBody(data._1.toSeq: _*),
              "ATED",
              Some(businessReg),
              overseasDetails,
              reviewDetails) { result =>
        ...
        "If updateNoRegister flag is set to true, must update registration rather then create a new one" in {
          when(mockBackLinkCache.saveBackLink(ArgumentMatchers.any(), ArgumentMatchers.any())(ArgumentMatchers.any(), ArgumentMatchers.any()))
            .thenReturn(Future.successful(None))
          updateWithAuthorisedUserSuccess(
            FakeRequest("POST", "/").withFormUrlEncodedBody(
              Map(
                "hasBusinessUniqueId" -> "true",
                "businessUniqueId"    -> "some-id",
  ...
  def viewWithUnAuthorisedUser()(test: Future[Result] => Any): Unit = {
    ...
    val result = TestController
      .view(serviceName, addClient = true)
      .apply(
        FakeRequest()
  ...
  def viewWithAuthorisedAgentWithSomeData(service: String)(test: Future[Result] => Any): Unit = {
    ...
    when(
      mockBusinessRegistrationCache.fetchAndGetCachedDetails[OverseasCompany](ArgumentMatchers.any())(ArgumentMatchers.any(), ArgumentMatchers.any()))
      .thenReturn(Future.successful(Some(OverseasCompany(Some(true)))))
  ...
  def viewWithAuthorisedUser(service: String, redirectUrl: Option[String] = None)(test: Future[Result] => Any): Unit = {
    ...
    val result = TestController
      .view(service, addClient = true, redirectUrl.map(RedirectUrl(_)))
      .apply(
        FakeRequest()
          .withSession("sessionId" -> sessionId, "token" -> "RANDOMTOKEN", "userId" -> userId)
          .withHeaders(Headers("Authorization" -> "value")))
  ...
  def registerWithAuthorisedUserSuccess(fakeRequest: FakeRequest[AnyContentAsFormUrlEncoded],
    ...
    when(
      mockBusinessRegistrationCache
        .fetchAndGetCachedDetails[Boolean](ArgumentMatchers.eq("Update_No_Register"))(ArgumentMatchers.any(), ArgumentMatchers.any()))
      .thenReturn(Future.successful(None))

    when(
      mockBusinessRegistrationCache.cacheDetails[OverseasCompany](ArgumentMatchers.any(), ArgumentMatchers.any())(
        ArgumentMatchers.any(),
        ArgumentMatchers.any(),
        ArgumentMatchers.any()))
    ...
    val result = TestController
      .register(service, addClient = true, redirectUrl.map(RedirectUrl(_)))
      .apply(
        fakeRequest
          .withSession("sessionId" -> sessionId, "token" -> "RANDOMTOKEN", "userId" -> userId)
          .withHeaders(Headers("Authorization" -> "value")))
  ...
  def updateWithAuthorisedUserSuccess(fakeRequest: FakeRequest[AnyContentAsFormUrlEncoded],
    ...
    when(
      mockBusinessRegistrationCache.fetchAndGetCachedDetails[BusinessRegistration](ArgumentMatchers.eq("BC_NonUK_Business_Details"))(
        ArgumentMatchers.any(),
        ArgumentMatchers.any()))
      .thenReturn(Future.successful(busRegCache))
