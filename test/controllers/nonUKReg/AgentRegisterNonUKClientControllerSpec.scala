...
class AgentRegisterNonUKClientControllerSpec extends GuiceTestApp with BeforeAndAfterEach {
  ...
  private val mockBusinessRegistrationCache: BusinessRegCacheService = mock[BusinessRegCacheService]
  private val mockBackLinkCache: BackLinkCacheService                = mock[BackLinkCacheService]
  ...
  "AgentRegisterNonUKClientController" must {
    ...
    "Authorised Users" must {
      ...
      "return business registration view for a Non-UK based client by agent with a back link" in new Setup {
        ...
        viewWithAuthorisedUser(service, "NUK", Some("/api/anywhere"), Some("http://localhost:9959/mandate/agent/inform-HMRC/nrl"), controller) {
          result =>
            status(result) must be(OK)
            val document = Jsoup.parse(contentAsString(result))
      ...
      "return business registration view for a Non-UK based client by agent with some saved data" in new Setup {
        ...
        when(mockBackLinkCache.saveBackLink(ArgumentMatchers.any(), ArgumentMatchers.any())(ArgumentMatchers.any(), ArgumentMatchers.any()))
          .thenReturn(Future.successful(Some("/api/anywhere")))
        viewWithAuthorisedUserWithSomeData(
          service,
          Some(businessReg),
          "NUK",
          Some("/api/anywhere"),
          Some("http://localhost:9959/mandate/agent/inform-HMRC/nrl"),
          controller) { result =>
          status(result) must be(OK)
          val document = Jsoup.parse(contentAsString(result))
      ...
      "validate form" must {
        ...
        "not be empty" in new Setup {
          submitWithAuthorisedUserSuccess(
            ...
            document
              ...
              .text() mustBe "Enter a business name Enter address line 1 Enter address line 2 Enter a country"
        ...
        "If registration details entered are valid, continue button must redirect to service specific redirect url" in new Setup {
          ...
          submitWithAuthorisedUserSuccess(
            ...
            "ATED",
            Some("http://localhost:9933/ated-subscription/registered-business-address"),
            controller
          ) { result =>
            status(result) must be(SEE_OTHER)
  ...
  def registerWithUnAuthorisedUser(businessType: String = "NUK", backLink: Option[String] = None, controller: AgentRegisterNonUKClientController)(
    ...
    val result = controller
      .view(service, backLink.map(RedirectUrl(_)))
      .apply(
        FakeRequest()
  ...
  def registerWithAuthorisedAgent(service: String,
                                  businessType: String,
                                  backLink: Option[String] = None,
                                  controller: AgentRegisterNonUKClientController)(test: Future[Result] => Any): Unit = {
    val sessionId = s"session-${UUID.randomUUID}"
    val userId    = s"user-${UUID.randomUUID}"
  ...
  def viewWithAuthorisedUser(service: String,
                             businessType: String,
                             backLink: Option[String] = None,
                             cachedBackLink: Option[String] = None,
                             controller: AgentRegisterNonUKClientController)(test: Future[Result] => Any): Unit = {
    val sessionId = s"session-${UUID.randomUUID}"
    val userId    = s"user-${UUID.randomUUID}"
    ...
    when(mockBackLinkCache.fetchAndGetBackLink(ArgumentMatchers.any())(ArgumentMatchers.any(), ArgumentMatchers.any()))
      .thenReturn(Future.successful(cachedBackLink))
  ...
  def viewWithAuthorisedUserWithSomeData(service: String,
                                         businessRegistration: Option[BusinessRegistration],
                                         businessType: String,
                                         backLink: Option[String] = None,
                                         cachedBackLink: Option[String] = None,
                                         controller: AgentRegisterNonUKClientController)(test: Future[Result] => Any): Unit = {
  ...
  def submitWithUnAuthorisedUser(businessType: String = "NUK",
                                 redirectUrl: Option[String] = Some("http://"),
                                 controller: AgentRegisterNonUKClientController)(test: Future[Result] => Any): Unit = {
    val sessionId = s"session-${UUID.randomUUID}"
    val userId    = s"user-${UUID.randomUUID}"
  ...
  def submitWithAuthorisedUserFailure(fakeRequest: FakeRequest[AnyContentAsJson],
                                      redirectUrl: Option[String] = Some("http://"),
                                      controller: AgentRegisterNonUKClientController)(test: Future[Result] => Any): Unit = {
    val sessionId = s"session-${UUID.randomUUID}"
    val userId    = s"user-${UUID.randomUUID}"
