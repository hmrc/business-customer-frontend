...
class ReviewDetailsControllerSpec
      ...
      "Authorised Users" must {
        ...
        "enrolling for EMAC" when {
          ...
          "return service start page correctly for ATED Users" in {
            ...
            continueWithAuthorisedUser(service) { result =>
              status(result) must be(SEE_OTHER)
          ...
          "return service start page correctly for AWRS Users" in {
            ...
            continueWithAuthorisedUser("AWRS") { result =>
              status(result) must be(SEE_OTHER)
          ...
          "redirect to the correspondence address page for capital-gains-tax-service" in {
            continueWithAuthorisedUser("capital-gains-tax") { result =>
              status(result) must be(SEE_OTHER)
              redirectLocation(result).get must include(
                "/capital-gains-tax/subscription/company/correspondence-address-confirm"
              )
            }
          }

          "redirect to the confirmation for agents address page for capital-gains-tax-subscription service" in {
            continueWithAuthorisedUser("capital-gains-tax-agents") { result =>
              status(result) must be(SEE_OTHER)
              redirectLocation(result).get must include(
                "/capital-gains-tax/subscription/agent/registered/subscribe"
              )
            }
          }

          "return agent registration page correctly for Agents" in {
            when(mockAgentRegistrationService.isAgentEnrolmentAllowed(ArgumentMatchers.eq(service)))
              .thenReturn(true)
            continueWithAuthorisedAgentEMAC(service) { result =>
              status(result) must be(SEE_OTHER)
          ...
          "return to error page for duplicate users" in {
            when(mockAgentRegistrationService.isAgentEnrolmentAllowed(ArgumentMatchers.eq(service)))
              .thenReturn(true)
            continueWithDuplicategentEmac("ATED") { result =>
          ...
          "return to error page for wrong role users" in {
            when(mockAgentRegistrationService.isAgentEnrolmentAllowed(ArgumentMatchers.eq(service)))
              .thenReturn(true)
            continueWithWrongRoleUserEmac("ATED") { result =>
              status(result) must be(OK)
              val document = Jsoup.parse(contentAsString(result))
              document.title() must be(
                "You must be logged in as an administrator to submit an ATED return - GOV.UK"
              )
            }
          }

          "respond with NotFound when invalid service is in uri" in {
            when(mockAgentRegistrationService.isAgentEnrolmentAllowed(ArgumentMatchers.eq("unknownServiceTest")))
              .thenReturn(false)
            intercept[NotFoundException] {
              continueWithAuthorisedUser("unknownServiceTest") { result =>
          ...
          "throw an exception for any invalid user" in {
            when(mockAgentRegistrationService.isAgentEnrolmentAllowed(ArgumentMatchers.eq(service)))
              .thenReturn(true)
            continueWithInvalidAgentEmac("ATED") { result =>
  ...
  private def continueWithUnAuthorisedUser(service: String)(test: Future[Result] => Any): Unit = {
    ...
    val result =
      testReviewDetailsController(nonDirectMatchReviewDetails).continue(service).apply(fakeRequestWithSession(userId))
    test(result)
  }
  ...
  private def continueWithAuthorisedUser(service: String)(test: Future[Result] => Any): Unit = {
    ...
    when(
      mockBackLinkCache.saveBackLink(ArgumentMatchers.any(), ArgumentMatchers.any())(
        ArgumentMatchers.any(),
        ArgumentMatchers.any()
      )
    ).thenReturn(Future.successful(None))
  ...
  private def continueWithAuthorisedAgentEMAC(service: String)(test: Future[Result] => Any): Unit = {
    ...
    when(
      mockAgentRegistrationService.enrolAgent(ArgumentMatchers.any())(
        ArgumentMatchers.any(),
        ArgumentMatchers.any(),
        ArgumentMatchers.any()
      )
  ...
  def businessDetailsWithUnAuthorisedUser(test: Future[Result] => Any): Unit = {
    ...
    val result =
      testReviewDetailsController(nonDirectMatchReviewDetails).businessDetails(service).apply(fakeRequestWithSession(userId))
