...
class ReviewDetailsNonUkAgentFeatureSpec extends GuiceFeatureApp with BeforeAndAfterEach with GivenWhenThen with Matchers {
  ...
  val reviewDetails: ReviewDetails = ReviewDetails(
    ...
    identification = Some(Identification("id", "inst", "FR"))
  )

  val reviewDetailsOverseas: ReviewDetails = ReviewDetails(
    "OverseasAgentRegistration",
    Some("NonUKBusinessType"),
    addressOverseas,
    "sap123",
    "safe123",
    directMatch = true,
    agentReferenceNumber = Some("agent123"),
    identification = Some(Identification("id", "inst", "CI"))
  )
  ...
  Feature("The user can view the review details page for a non uk agent") {
    ...
    Scenario("return Review Details view for an agent, when agent can't be directly found with login credentials and the reg is editable") {
      ...
      assert(
        document.getElementById("check-agency-details").text ===
