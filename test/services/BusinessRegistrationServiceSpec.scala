...
class BusinessRegistrationServiceSpec extends GuiceTestApp with BeforeAndAfterEach {
  ...
  object TestBusinessRegistrationService
      extends BusinessRegistrationService(
  ...
  "registerBusiness" must {
    val nonUKResponse = BusinessRegistrationResponse(
    ...
    "save the response from the registration" in {
      ...
      when(
        mockBusinessCustomerConnector.register(ArgumentMatchers.any(), ArgumentMatchers.any(), ArgumentMatchers.any())(
          ArgumentMatchers.any(),
          ArgumentMatchers.any(),
          ArgumentMatchers.any())).thenReturn(Future(nonUKResponse))
      ...
      val returnedReviewDetails = new ReviewDetails(
        businessName = busRegData.businessName,
        businessType = None,
        businessAddress = busRegData.businessAddress,
        sapNumber = "sap123",
        safeId = "safe123",
        isAGroup = false,
        agentReferenceNumber = Some("agent123"))
      when(mockDataCacheConnector.saveReviewDetails(ArgumentMatchers.any())(ArgumentMatchers.any(), ArgumentMatchers.any()))
        .thenReturn(Future.successful(Some(returnedReviewDetails)))

      val regResult =
        TestBusinessRegistrationService.registerBusiness(busRegData, overseasCompany, isGroup = true, isNonUKClientRegisteredByAgent = false, service)
    ...
    "save the response from the registration, when an agent registers non-uk based client" in {
      ...
      val regResult =
        TestBusinessRegistrationService.registerBusiness(busRegData, overseasCompany, isGroup = true, isNonUKClientRegisteredByAgent = true, service)
  ...
  "updateBusiness" must {
    ...
    val cachedReviewDetails = ReviewDetails(
      businessName = "ABC",
      businessType = Some("corporate body"),
      businessAddress = Address(line_1 = "line1", line_2 = "line2", line_3 = None, line_4 = None, postcode = None, country = "GB"),
      sapNumber = "1234567890",
    ...
    "save the response from the update" in {
      ...
      when(
        mockBusinessCustomerConnector.updateRegistrationDetails(ArgumentMatchers.any(), ArgumentMatchers.any())(
          ArgumentMatchers.any(),
          ArgumentMatchers.any(),
          ArgumentMatchers.any())).thenReturn(Future.successful(HttpResponse(OK, successResponse.toString)))
      ...
      val regResult = TestBusinessRegistrationService.updateRegisterBusiness(
        busRegData,
        overseasCompany,
        isGroup = true,
