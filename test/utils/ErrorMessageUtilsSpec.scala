  ...
  "ErrorMessageUtils" must {
    "return true" when {
      "error number is 9001 and message is duplicate agent error" in {
        val badGatewayResponse = Json.parse(
        ...
        matchErrorResponse(HttpResponse(502, badGatewayResponse.toString)) must be(true)
      }
    }
    ...
    "return false" when {
      "error number is 9002 and message is duplicate agent error" in {
        ...
        matchErrorResponse(HttpResponse(502, badGatewayResponse.toString)) must be(false)
      }
    }
    ...
    "Show that scala.xml.XML can protect against file access when securely configured" in {
      intercept[SAXParseException] {
        ...
        matchErrorResponse(HttpResponse(502, payload.toString))
      }
    }
  }
